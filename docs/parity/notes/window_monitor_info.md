# monitor_info Parity Note

- Bevy source: bevy/examples/window/monitor_info.rs
- mgstudio target: mgstudio-engine/examples/window/monitor_info/
...
- Monitor enumeration/details API is not exposed in current API.
...
- Build command: moon -C mgstudio-engine build --release --target wasm examples/window/monitor_info
- Smoke command: moon -C mgstudio-engine check
- Logs: window.host_debug_string with [monitor_info] prefix.

## Follow-up Tasks

- Remaining blockers: Monitor enumeration/details API is not exposed in current API.
