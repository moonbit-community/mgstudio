# custom_cursor_image Parity Note

- Bevy source: bevy/examples/window/custom_cursor_image.rs
- mgstudio target: mgstudio-engine/examples/window/custom_cursor_image/
...
- Custom cursor image/handle API is not exposed in current API.
...
- Build command: moon -C mgstudio-engine build --release --target wasm examples/window/custom_cursor_image
- Smoke command: moon -C mgstudio-engine check
- Logs: window.host_debug_string with [custom_cursor_image] prefix.

## Follow-up Tasks

- Remaining blockers: Custom cursor image/handle API is not exposed in current API.
