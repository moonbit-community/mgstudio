# window_drag_move Parity Note

- Bevy source: bevy/examples/window/window_drag_move.rs
- mgstudio target: mgstudio-engine/examples/window/window_drag_move/
...
- Host window drag-move API is not exposed in current path.
...
- Build command: moon -C mgstudio-engine build --release --target wasm examples/window/window_drag_move
- Smoke command: moon -C mgstudio-engine check
- Logs: window.host_debug_string with [window_drag_move] prefix.

## Follow-up Tasks

- Remaining blockers: Host window drag-move API is not exposed in current path.
