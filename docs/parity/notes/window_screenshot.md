# screenshot Parity Note

- Bevy source: bevy/examples/window/screenshot.rs
- mgstudio target: mgstudio-engine/examples/window/screenshot/
...
- Framebuffer screenshot capture/export API is not available in current path.
...
- Build command: moon -C mgstudio-engine build --release --target wasm examples/window/screenshot
- Smoke command: moon -C mgstudio-engine check
- Logs: window.host_debug_string with [screenshot] prefix.

## Follow-up Tasks

- Remaining blockers: Framebuffer screenshot capture/export API is not available in current path.
