...
| RENDER-004 | bevy_pbr/render/mesh.rs | Mesh extract path is not changed/removed incremental at Bevy granularity. | IN_PROGRESS | 2026-04-07: landed mesh extraction cache + changed/removed/reextract incremental refresh in `pbr/render3d.mbt`, and skip payload rebuild when motion-vector history is not needed; `many_foxes` no-trace run (`/tmp/mgstudio_stress_profile_render_parity_r004_notrace3`) is ~`6.67 fps`, trace run (`/tmp/mgstudio_stress_profile_render_parity_r004_trace5`) keeps `mgstudio.render3d.extract#2` around ~`32.659ms`; still not parity-complete. |
