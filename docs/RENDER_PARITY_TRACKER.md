...
| RENDER-004 | bevy_pbr/render/mesh.rs | Mesh extract path is not changed/removed incremental at Bevy granularity. | IN_PROGRESS | 2026-04-07: added mesh extraction cache + changed/removed/reextract incremental refresh path in `pbr/render3d.mbt`; `many_foxes` trace run (`/tmp/mgstudio_stress_profile_render_parity_r004_trace3`) shows `mgstudio.render3d.extract#2` avg reduced to ~`31.401ms` (from ~`34.172ms` in trace2), still not parity-complete. |
