...
| RENDER-010 | bevy_pbr/render/mesh.rs | Queue/execute still carries non-queue responsibilities (build-time material readiness retries). | IN_PROGRESS | 2026-04-12: moved camera/mesh collection and frame assembly from `render3d_queue_system` into `render3d_prepare_meshes_system` (PrepareMeshes set); next step is to push remaining queue-build ownership out of execute camera passes. |
...
| RENDER-012 | bevy_render schedule + bevy_pbr/render/mod.rs | Render schedule set topology differs from Bevy (`Extract/Prepare*/Queue/Render/Cleanup`). | IN_PROGRESS | 2026-04-12: set families are wired in `pbr/mesh_material.mbt`; `PrepareResourcesFlush` now clears stale queued-frame state, mesh collection moved to `PrepareMeshes`, and bind-group preparation has an explicit `PrepareBindGroups` entrypoint; remaining work is full preprocess-flush and bind-group ownership migration. |
