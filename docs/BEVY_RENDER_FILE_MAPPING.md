...
- [x] `bevy/crates/bevy_pbr/src/render/mesh_view_bindings.rs` (projection matrix shape + matrix ops + temporal jitter + ortho/perspective raw helpers + frustum-corner helper) -> `mgstudio-engine/pbr/render/projection_types.mbt` (`Migrated ProjectionMatrix3d ownership + row/inverse + temporal-jitter + near-from-clip + orthographic-matrix + perspective-raw + frustum-corners`, root `core_pipeline.mbt` now aliases matrix type; root `render3d_view_bindings` bridges these helpers)
