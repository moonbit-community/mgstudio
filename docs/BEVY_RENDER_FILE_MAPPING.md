...
- [x] `bevy/crates/bevy_pbr/src/render/mesh_bindings.rs` (begin-pass host binding path) -> `mgstudio-engine/pbr/render/mesh_bindings.mbt` (`Migrated render3d_begin_pass_3d implementation`, root `render3d_bindings.mbt` now bridges)
- [x] `bevy/crates/bevy_pbr/src/render/mesh_bindings.rs` (mesh draw payload shape + build/draw host binding path) -> `mgstudio-engine/pbr/render/mesh_bindings.mbt` (`Migrated MeshGpuDrawPayload3d ownership + render3d_build_mesh_gpu_draw_payload + render3d_draw_mesh_gpu_payload`, root `render3d_bindings.mbt` now bridges; root `phase.mbt` now aliases payload type)
...
- [x] `bevy/crates/bevy_pbr/src/render/mesh.rs` (material cull-mode encoding helper) -> `mgstudio-engine/pbr/render/material_flags.mbt` (`Migrated render3d_cull_mode_value implementation`, root `render3d_bindings.mbt` now bridges)
