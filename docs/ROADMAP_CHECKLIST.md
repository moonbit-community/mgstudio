...
- [ ] Issue `render/bevy_stage_boundary_parity`: `Extract/Prepare/Queue/Execute` entrypoints已独立，`execute_impl` 已迁至 `render_execute`，camera queue-build/motion-vector/main-pass/point-shadow/postprocess 已独立；pending按 Bevy 文件边界继续细化 execute orchestration 层。
