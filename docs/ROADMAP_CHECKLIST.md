...
- [ ] Issue `pbr/render_projection_type_ownership`: `Projection/PerspectiveProjection3d/OrthographicProjection3d` and related matrix/view helpers remain parent-owned in `pbr/core_pipeline.mbt` + `render3d_view_bindings.mbt`, blocking full migration of `render3d_view_bindings` and `render3d_bindings` implementations into `pbr/render`.
