...
| `bevy_core_pipeline` | `mgstudio-engine/core_pipeline` | 🟡 In Progress | 91% | Most node/plugin shapes landed; package tests/check are green (`moon test core_pipeline --target native`, `moon check core_pipeline --target native`) but some runtime parity is still shallow. | Finish remaining runtime-depth parity items. |
