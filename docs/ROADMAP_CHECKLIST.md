...
| `bevy_pbr` (overall) | `mgstudio-engine/pbr` | 95% | 94% | 94% | 🟡 In Progress | Remaining parity gaps concentrate in light/fog/meshlet/deferred depth behavior. |
...
| `bevy_light` | `mgstudio-engine/light` | 93% | 76% | 76% | 🟡 In Progress | Light clustering/runtime integration still has parity-tail differences. |
...
| `bevy_a11y` | `mgstudio-engine/a11y` | 95% | 90% | 90% | 🟡 In Progress | Final semantic parity audit against Bevy accessibility behavior is pending. |
...
- [x] `pbr/render-cluster-factor-formula`: align `cluster_factors.zw` computation with Bevy `calculate_cluster_factors` formula path and wire shared helper from `light/cluster`.
...
- [x] `a11y/resource-shared-flag-semantics`: add explicit clone/shared-handle semantics tests for `AccessibilityRequested` and mutation coverage for `ManageAccessibilityUpdates`.
