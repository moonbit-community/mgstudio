...
| `bevy_pbr::prepass` | `mgstudio-engine/pbr/prepass` | 94% | 88% | 88% | 🟡 In Progress | Remaining pass ordering/bind-group lifecycle needs Bevy-level matching. |
...
- [x] `pbr/prepass`: replace queue-state camera/mesh motion-blur history caches with ECS-owned previous-frame components (`PreviousViewData`, `PreviousGlobalTransform`), centralize write-back at execute tail, and sync previous mesh transform for all Mesh3d entities (not just extracted visible set).
