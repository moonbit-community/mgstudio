...
| `bevy_pbr::meshlet` | `mgstudio-engine/pbr/meshlet` | 94% | 75% | 75% | 🟡 In Progress | Runtime state semantics + render/core3d stage ownership are aligned; shader/pipeline-specialization depth is still simplified vs Bevy. |
...
- [x] `render/pbr`: close `RENDER-012` by wiring concrete work to staged `prepare meshes -> prepare bind groups -> queue -> execute` topology, removing queued-frame take/store ping-pong in prepare-bind-groups/queue-meshes, prewarming mesh3d pipeline variants during prepare-meshes, feeding prewarm with extracted mesh topology (no mesh-id reverse lookup in hot path), deduping prewarm calls by pipeline key, reusing extracted `MeshPreprocessStatic3d` payload in queue/preprocess paths, giving queue-sweep concrete mesh-runtime-cache cleanup responsibility, removing transient queue-result rebuilds in upload/dispatch paths, storing prepared camera queues in single-pass frame assembly, precomputing frame camera-loop bounds once, and sharing prepared-camera iteration logic across prepare/queue consumers.
