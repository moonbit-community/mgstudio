...
| `bevy_asset` | `mgstudio-engine/asset` | 94% | 90% | 90% | 🟡 In Progress | Native asset runtime is stable with capability-gated HTTP fetch; image extension routing now derives from `bevy_image`-shaped loader tables and image meta settings (including sampler compare/border/anisotropy) are wired; remaining gaps focus on broader codec parity depth. |
...
| `bevy_image` | `mgstudio-engine/image` | 93% | 85% | 85% | 🟡 In Progress | `ImageLoader` type + extensions/supported-format tables + expanded magic-byte guess path + Bevy-shaped loader error surfaces moved closer; BMP decode/runtime dimensions are wired, uncompressed TGA/PNM decode run through container-aware asset host path, and farbfeld/QOI decode + dimensions are now supported, with remaining codec parity still open in constrained environments. |
