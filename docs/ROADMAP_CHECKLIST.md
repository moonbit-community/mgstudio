...
- [ ] Issue `pbr/render_subpackage_topology`: landed `pbr/render` subpackage topology (`top/systems/view/diagnostic/mesh/light/fog/skin/morph/mesh_bindings/mesh_view_bindings/gpu_preprocess/targets/runtime_config/math/projection_runtime/gpu_preprocess_types/phase_state/world_transform_cache`), and migrated `render3d_targets` + runtime-config helpers + `render3d_math` + `render3d_light` + projection/runtime preprocess type ownership + phase-state resource + world-transform-cache + skin extraction/upload path + diagnostic runtime state/accessors into subpackage; pending remaining `render3d_*` implementation migration.
