...
| `bevy_pbr` (overall) | `mgstudio-engine/pbr` | 95% | 89% | 89% | 🟡 In Progress | Remaining parity gaps concentrate in light/fog/meshlet/deferred depth behavior. |
| `bevy_pbr::render` | `mgstudio-engine/pbr/render` | 96% | 91% | 91% | 🟡 In Progress | Core stage split is landed; remaining gaps are meshlet/advanced pass feature depth. |
...
| Current weighted migration completion (included scope) | 87% |
