...
| `bevy_render::renderer` internals | `mgstudio-engine/render/renderer` | 🟡 In Progress | 86% | Backend runtime API split progressed (`top` wrappers and `mesh3d` public runtime APIs moved out of monolithic owners); deeper pipeline/material decomposition remains. | Continue splitting `mesh3d_draw` runtime/resource owners by Bevy responsibilities. |
