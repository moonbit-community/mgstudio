...
| `bevy_pbr::prepass` | `mgstudio-engine/pbr/prepass` | 94% | 86% | 86% | 🟡 In Progress | Remaining pass ordering/bind-group lifecycle needs Bevy-level matching. |
...
- [x] `pbr/prepass`: replace queue-state camera/mesh motion-blur history caches with ECS-owned previous-frame components (`PreviousViewData`, `PreviousGlobalTransform`) and centralize write-back at execute tail instead of scattered camera/mesh paths.
