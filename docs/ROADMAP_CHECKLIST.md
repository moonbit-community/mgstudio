...
| `bevy_render` (topology) | `mgstudio-engine/render` | 🟡 In Progress | 85% | File topology is close; `renderer` owner splits continue to remove monolithic runtime files (`mesh3d_preprocess_types`, `mesh3d_preprocess_shader_load`, `mesh3d_preprocess_resources`). | Continue file-by-file owner convergence. |
| `bevy_render::renderer` internals | `mgstudio-engine/render/renderer` | 🟡 In Progress | 91% | Backend runtime API split progressed (`top` wrappers + `mesh3d` public runtime APIs), plus pipeline-cache, bind-group/material-cache, pipeline-build, and preprocess shader/resource owners split from monolithic `mesh3d_draw`; deeper preprocess dispatch/runtime decomposition remains. | Continue splitting `mesh3d_draw` preprocess dispatch/runtime owners by Bevy responsibilities. |
