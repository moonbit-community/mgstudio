...
| `bevy_light` | `mgstudio-engine/light` | 93% | 77% | 77% | 🟡 In Progress | Light clustering/runtime integration still has parity-tail differences. |
...
| `bevy_a11y` | `mgstudio-engine/a11y` | 95% | 92% | 92% | 🟡 In Progress | Final semantic parity audit against Bevy accessibility behavior is pending. |
...
- [x] `a11y/resource-shared-flag-semantics`: add explicit clone/shared-handle semantics tests for `AccessibilityRequested`, mutation coverage for `ManageAccessibilityUpdates`, and `init_resource` keep-existing semantics parity.
