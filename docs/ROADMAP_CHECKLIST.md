...
| `bevy_render::renderer` internals | `mgstudio-engine/render/renderer` | 🟡 In Progress | 87% | Backend runtime API split progressed (`top` wrappers + `mesh3d` public runtime APIs) and pipeline-cache owner split landed; deeper preprocess/pipeline/material decomposition remains. | Continue splitting `mesh3d_draw` preprocess/pipeline creation owners by Bevy responsibilities. |
