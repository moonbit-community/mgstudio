...
| `bevy_render` (topology) | `mgstudio-engine/render` | 🟡 In Progress | 83% | File topology is close; `renderer` owner splits continue to remove monolithic runtime files. | Continue file-by-file owner convergence. |
| `bevy_render::renderer` internals | `mgstudio-engine/render/renderer` | 🟡 In Progress | 89% | Backend runtime API split progressed (`top` wrappers + `mesh3d` public runtime APIs), plus pipeline-cache, bind-group/material-cache, and pipeline-build owners split from monolithic `mesh3d_draw`; deeper preprocess dispatch/resource decomposition remains. | Continue splitting `mesh3d_draw` preprocess dispatch/resource owners by Bevy responsibilities. |
