...
| `bevy_pbr::meshlet` | `mgstudio-engine/pbr/meshlet` | 93% | 68% | 68% | 🟡 In Progress | Runtime state semantics + render3d set ownership are aligned; view/material/pass pipeline depth is still partial. |
...
- [x] `pbr/meshlet`: align render-sub-app scheduling to Bevy-shaped stage ownership (`extract/prepare_meshes/prepare_resources/prepare_bind_groups/queue`) using shared render3d system-set names.
