...
| `bevy_asset` | `mgstudio-engine/asset` | 94% | 94% | 94% | 🟡 In Progress | Native asset runtime is stable with capability-gated HTTP fetch; image extension routing now derives from `bevy_image`-shaped loader tables and image meta settings (including sampler compare/border/anisotropy) are wired; remaining gaps focus on broader codec parity depth. |
...
| `bevy_image` | `mgstudio-engine/image` | 93% | 91% | 91% | 🟡 In Progress | `ImageLoader` type + extensions/supported-format tables + expanded magic-byte guess path + Bevy-shaped loader error surfaces moved closer; BMP decode/runtime dimensions are wired, TGA (including RLE) and PNM (binary + ascii) decode run through container-aware host path, farbfeld/QOI/ICO(PNG+DIB32 payload) decode + dimensions are supported, and GIF/WEBP/TIFF dimension parsing is wired for non-decoded fallbacks; remaining codec parity is still open. |
