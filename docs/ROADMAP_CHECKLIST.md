...
| `bevy_pbr::prepass` | `mgstudio-engine/pbr/prepass` | 94% | 84% | 84% | 🟡 In Progress | Remaining pass ordering/bind-group lifecycle needs Bevy-level matching. |
...
- [x] `pbr/prepass`: replace queue-state camera/mesh motion-blur history caches with ECS-owned previous-frame components (`PreviousViewData`, `PreviousGlobalTransform`) and write-back on render execution path.
