...
| `render/renderer` | 52% | 47% | 47% | 🔴 Major Gap | Too much local renderer architecture remains, but mesh3d allocator-backed uploads now have a real deletion path that frees renderer mesh records and unused slab buffers. Renderer ownership, render-graph integration, pass layout, and visual parity still diverge substantially. |
...
| `mesh` | 80% | 74% | 74% | 🟡 In Progress | Primitive, attribute, vertex-layout, allocator-state, upload-descriptor surfaces, render-mesh allocation planning, renderer mesh3d slab upload execution, Bevy-shaped free-before-reallocate deletion, and PBR preprocess offset consumption are closer to Bevy. Remaining runtime work is broader indirect/preprocess draw parity and complete extracted-asset scheduling equivalence. |
