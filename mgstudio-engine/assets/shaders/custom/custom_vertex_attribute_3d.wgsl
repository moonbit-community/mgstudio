...
fn vs_main(
  ...
  let world_pos = quat_rotate_vec3(model_q, local_pos) + draw.transform.model_translation.xyz;
...
fn fs_main(in : VertexOut) -> @location(0) vec4<f32> {
  ...
  let pulse = 0.6 + 0.4 * (0.5 + 0.5 * sin(draw.material.reflectance * 12.0));
  var rgb = in.vertex_color.rgb * draw.material.base_color.rgb * pulse;
  let checker = select(0.85, 1.15, fract(in.uv.x * 10.0 + in.uv.y * 10.0) > 0.5);
  rgb = rgb * checker;
  let alpha = in.vertex_color.a * draw.material.base_color.a;
  if material_flag_enabled(draw.material.flags, STANDARD_MATERIAL_FLAGS_ALPHA_MODE_MASK) &&
    alpha < draw.material.alpha_cutoff {
    discard;
  }
