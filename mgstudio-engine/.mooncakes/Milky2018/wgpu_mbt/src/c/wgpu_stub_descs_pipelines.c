...
uint32_t mbt_wgpu_render_pipeline_desc_builder_set_front_face(void *builder,
                                                              uint32_t front_face_u32) {
  if (!builder) {
    return MBT_WGPU_RP_ERR_NULL_BUILDER;
  }
  mbt_render_pipeline_desc_t *out = (mbt_render_pipeline_desc_t *)builder;
  mbt_wgpu_rp_builder_clear_error(out);
  out->primitive.frontFace = (WGPUFrontFace)front_face_u32;
  out->desc.primitive.frontFace = (WGPUFrontFace)front_face_u32;
  return MBT_WGPU_RP_OK;
}
