...
struct MeshletRemapDispatchConstants {
    max_compute_workgroups_per_dimension: u32,
}

@group(0) @binding(2) var<uniform> meshlet_remap_dispatch_constants: MeshletRemapDispatchConstants;

@compute
@workgroup_size(1, 1, 1)
fn remap_dispatch() {
    let cluster_count = meshlet_software_raster_indirect_args.x;
    let max_compute_workgroups_per_dimension = meshlet_remap_dispatch_constants.max_compute_workgroups_per_dimension;
